...
std::vector<std::vector<float>> matadd_bias(std::vector<std::vector<float>> vecA, std::vector<std::vector<float>> vecB){
	if(vecA[0].size() != vecB[0].size())
		throw std::runtime_error("Invalid matrix dimensions!");
	
	std::vector<std::vector<float>> sum (vecA.size(), std::vector<float>(vecB[0].size()));
	for(int r = 0; r < vecA.size(); r++){
		for(int c = 0; c < vecA[0].size(); c++){
			sum[r][c] = vecA[r][c] + vecB[0][c];
		}
	}
...
std::vector<std::vector<float>> matsub(std::vector<std::vector<float>> vecA, float cons){
	std::vector<std::vector<float>> sum (vecA.size(), std::vector<float>(vecA[0].size()));
	for(int r = 0; r < vecA.size(); r++){
		for(int c = 0; c < vecA[0].size(); c++){
			sum[r][c] = vecA[r][c] - cons;
